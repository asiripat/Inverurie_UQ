...
# Step 1: Get all the files that contain 'violin_plot_data' and '2D' in their filenames
file_pattern = '*violin_plot_data*2D*.csv'
file_list = glob.glob(file_pattern)

# Sanity check: ensure 9 files are found
assert len(file_list) == 9, f"Expected 9 files, but found {len(file_list)}."
...
for file in file_list:
    ...
    # Remove 'violin plot data' and '2D' from the label
    filename_clean = filename.replace('violin plot data', '').replace('2D', '').strip()

    # Modify 'lhc' to uppercase 'LHC' in the label
    filename_clean = filename_clean.replace('lhc', 'LHC')

    # Add a 'Dataset' column to distinguish each file in the combined plot
    data['Dataset'] = filename_clean
...
sns.violinplot(x='Dataset', y='Value', hue='Type', data=all_data, palette="Set2", split=True, inner="quart", gap=0.1)

# Step 4: Customize the plot
plt.title('Full MC Data vs Surrogate Estimates (Two-input Case)', fontsize=16)
...
plt.tight_layout()
#plt.savefig('comparison_violin_plot_2D.png', dpi=300)  # Save as PNG with 300 dpi resolution
